import type { AgreementActionId } from '../shared/components/agreementRowActions'
...
export type AgreementStatus = 'Pré-preenchido' | 'Conferido' | 'Exceção' | 'Aguardando DI' | 'Manual pendente' | 'Apurado DI' | 'Valor real informado' | 'Com divergência' | 'Confirmado KAM' | 'Aguardando aprovação' | 'Aprovado' | 'Ajuste solicitado' | 'Aguardando assinatura' | 'Assinado' | 'PO emitido' | 'NF recebida' | 'NF validada' | 'Pagamento solicitado' | 'Documento SAP gerado' | 'Pago' | 'Saldo em aberto' | 'Pagamento rejeitado' | 'Devolvido' | 'Finalizado'
...
export interface Lever { id:string; network:string; buyer:string; kam:string; category:string; name:string; observation:string; agreementType:AgreementType; automation:AutomationType; annualTarget:number; annualBudget:number; allocated:number; committed:number; paid:number; status:'Ativo'|'Pendente'|'Com erro'|'Inativo' }
export interface AuditEvent { at:string; user:string; action:string; description:string; changedFields?:string[] }
export interface Agreement { id:string; leverId:string; month:string; network:string; buyer:string; kam:string; ppm:string; lever:string; leverObservation:string; agreementType:AgreementType; poStatus?:'pending'|'issued'; invoiceStatus?:'pending'|'received'|'validated'; automation:AutomationType; plannedTarget:number; monthlyTarget:number; kamAdjustedTarget?:number; budget:number; diValue?:number; realValue?:number; proposedValue?:number; approvedValue?:number; paidValue?:number; stage:Stage; status:AgreementStatus; exceptionReason?:string; kamConfirmed?:string; ppmDecision?:string; signatureStatus?:string; sapDocument?:string; sapStatus?:string; evidence?:string; updatedAt:string; customFields?:Record<string,string|number|boolean>; auditTrail?:AuditEvent[] }
...
export type Action =
  ...
  | {type:'CONFIRM_KAM'; ids:string[]} | {type:'SEND_APPROVAL'; ids:string[]} | {type:'EXECUTE_AGREEMENT_ACTION'; ids:string[]; actionId:AgreementActionId; user:'KAM'|'PPM'}
