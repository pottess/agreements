...
export interface Agreement { id:string; leverId:string; month:string; network:string; buyer:string; kam:string; ppm:string; lever:string; automation:AutomationType; plannedTarget:number; monthlyTarget:number; budget:number; diValue?:number; realValue?:number; proposedValue?:number; approvedValue?:number; paidValue?:number; stage:Stage; status:AgreementStatus; exceptionReason?:string; kamConfirmed?:string; ppmDecision?:string; signatureStatus?:string; sapDocument?:string; sapStatus?:string; evidence?:string; updatedAt:string; customFields?:Record<string,string|number|boolean>; auditTrail?:AuditEvent[] }
