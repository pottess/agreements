...
export function MyAgreementsPage() {
  ...
  const columns: Column<Agreement>[] = [
    ...
    { key: 'measured', label: 'Apurado', render: agreement => agreement.automation === 'DI' ? agreement.diValue === undefined ? <span>Aguardando DI</span> : brl(agreement.diValue) : agreement.measurementType === 'qualitative' && current ? <InlineOkToggle checked={agreement.qualitativeResult==='ok'} onToggle={checked => dispatch({type:'TOGGLE_QUALITATIVE_OK', id:agreement.id, checked})}/> : agreement.measurementType === 'qualitative' ? <span>{agreement.qualitativeResult==='ok'?'OK':'—'}</span> : current ? <InlineCurrencyInput value={agreement.diValue} onSave={value => dispatch({type:'SET_APURADO_VALUE', id:agreement.id, value})}/> : agreement.diValue === undefined ? <span>—</span> : brl(agreement.diValue) },
